...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // =====================================================
    // CURSOS — CORREÇÃO DE UNIDADE EM LOTE
    // =====================================================
    ...
    if (url.pathname === "/api/cursos" && request.method === "GET") {
      ...
      const cursos = new Map<
        string,
        { curso: string; total_alunos: number; unidades: Array<{ unidade: string; total: number }> }
      >();
      ...
      for (const item of resultado.results) {
        ...
        atual.unidades.push({ unidade: item.unidade, total: Number(item.total) });
    ...
    if (url.pathname === "/api/cursos/unidade" && request.method === "PUT") {
      ...
      const resultados = await env.DB.batch([
        ...
        ).bind(unidade, periodoAtual!.id, curso, unidade),
        env.DB.prepare(
          `INSERT INTO google_sheets_mapeamentos
           ...
           ON CONFLICT(periodo_id, curso_chave) DO UPDATE SET
             ...
             atualizado_em = CURRENT_TIMESTAMP`
        ).bind(periodoAtual!.id, cursoChave, curso, unidade),
      ]);

      const alterados = Number(resultados[0].meta.changes ?? 0);
      await registrarAuditoria(env, usuarioAtual!, periodoAtual!.id, {
        acao: "MAPEAR_UNIDADE",
        entidade: "CURSO",
