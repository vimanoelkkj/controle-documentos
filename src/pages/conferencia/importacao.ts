...
export function analisarTextoImportacao(
  ...
  const alunos = linhas.slice(1).map((linha, indice) => {
    ...
    const email =
      indiceEmail >= 0 ? colunas[indiceEmail]?.trim() || undefined : undefined;
    const email_outro =
      indiceEmailOutro >= 0
  ...
  return alunos.map((aluno) => {
    ...
    if (normalizarCampo(alunoSalvo.nome) !== normalizarCampo(aluno.nome))
      alteracoes.push(`Nome: ${alunoSalvo.nome} → ${aluno.nome}`);
    if (normalizarCampo(alunoSalvo.curso) !== normalizarCampo(aluno.curso))
      alteracoes.push(`Curso: ${alunoSalvo.curso} → ${aluno.curso}`);
    if (
      normalizarCampo(alunoSalvo.unidade) !== normalizarCampo(unidadeImportacao)
    )
      alteracoes.push(`Unidade: ${alunoSalvo.unidade} → ${unidadeImportacao}`);
    if (normalizarCampo(alunoSalvo.email) !== normalizarCampo(aluno.email))
      alteracoes.push(
        `E-mail: ${alunoSalvo.email || "—"} → ${aluno.email || "—"}`,
      );
    if (
      normalizarCampo(alunoSalvo.email_outro) !==
      normalizarCampo(aluno.email_outro)
    )
      alteracoes.push(
        `E-mail alternativo: ${alunoSalvo.email_outro || "—"} → ${aluno.email_outro || "—"}`,
      );
