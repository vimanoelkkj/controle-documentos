...
function cursoLabel(s: string) {
  return s
    .replace(
      /^(EAD|SEMIPRESENCIAL|BACHARELADO EM|TECNOLOGIA EM)\s*[-–—:]?\s*/i,
      "",
    )
    .replace(/\s+(EAD|SEMIPRESENCIAL)$/i, "")
    .trim()
    .toUpperCase();
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch("/api/alunos", { cache: "no-store" })
      .then((r) => {
        ...
        return r.json() as Promise<AlunoApi[]>;
      })
      .then((d) => ativo && setAlunos(d))
      .catch(
        () =>
          ativo && setErro("Não foi possível carregar os dados do dashboard."),
      );
    return () => {
      ativo = false;
    };
  ...
  useEffect(() => {
    if (!unidades.some((u) => u.u === unidade && u.total > 0))
      setUnidade(unidades.find((u) => u.total > 0)?.u || "EAD");
  }, [unidades, unidade]);
  ...
  const cursos = useMemo(() => {
    ...
    ativos
      ...
      .forEach((a) => m.set(a.curso, (m.get(a.curso) || 0) + 1));
    return [...m]
  ...
  const pendencias = useMemo(
    () =>
      DOCS.map((d) => ({
        ...d,
        total: ativos.filter((a) => a[d.campo] !== 1).length,
      }))
        .sort((a, b) => b.total - a.total)
        .slice(0, 4),
    [ativos],
  ...
  return (
    <>
      ...
      <div className="page-head">
        <h1>Dashboard</h1>
        <p>Visão geral da documentação dos alunos por unidade.</p>
      </div>
      <section>
        <div className="stat-grid">
          <div className="stat-tile">
            ...
            <div className="stat-sub">100% do total</div>
          </div>
          <div className="stat-tile">
            <div className="stat-icon teal">✓</div>
            <div className="stat-value">{fmt(resumo.completo)}</div>
            <div className="stat-label">Completos</div>
            <div className="stat-sub">
              {pct(resumo.completo, total).toLocaleString("pt-BR", {
          ...
          <div className="stat-tile">
            ...
            <div className="stat-value">{fmt(resumo.parcial)}</div>
            <div className="stat-label">Parciais</div>
            <div className="stat-sub">
              {pct(resumo.parcial, total).toLocaleString("pt-BR", {
                maximumFractionDigits: 1,
              })}
              % do total
            </div>
          ...
          <div className="stat-tile">
            <div className="stat-icon terracotta">✕</div>
            <div className="stat-value">{fmt(resumo.critico)}</div>
            <div className="stat-label">Críticos</div>
            <div className="stat-sub">
              {pct(resumo.critico, total).toLocaleString("pt-BR", {
                maximumFractionDigits: 1,
              })}
              % do total
            </div>
          ...
          <div className="stat-tile">
            ...
            <div className="stat-value">
              {conclusao.toLocaleString("pt-BR", { maximumFractionDigits: 1 })}%
            </div>
            <div className="stat-label">Conclusão geral</div>
      ...
      <section>
        <div className="section-head">
          <div>
            ...
            <p className="section-sub">
              Selecione uma unidade para visualizar a distribuição por curso.
            </p>
          </div>
        </div>
        <div className="unit-grid">
          {unidades.map((x) => (
            <div
              key={x.u}
              className={`unit-card${unidade === x.u ? " selected" : ""}`}
              onClick={() => setUnidade(x.u)}
            >
              <div className="u-tag">{x.u}</div>
              <div className="u-value">{fmt(x.total)}</div>
              <div className="u-pct">
                {pct(x.total, total).toLocaleString("pt-BR", {
                  maximumFractionDigits: 1,
                })}
                % do total
              </div>
      ...
      <section>
        <div className="section-head">
          <div>
            <h2>{unidade}</h2>
            <p className="section-sub">
              {fmt(unidades.find((u) => u.u === unidade)?.total || 0)} alunos
              distribuídos em {cursos.length} cursos
            </p>
          </div>
        </div>
        <div className="bar-scroll">
          <div className="bar-chart" id="barChart">
            {cursos.map((c) => (
              <div className="bar-col" key={c.nome}>
                <div className="bar-val">{c.total}</div>
                <div
                  className="bar"
                  style={{
                    height: `${Math.max(4, (c.total / maxCurso) * 175)}px`,
                  }}
                />
              </div>
            ))}
          </div>
          <div className="bar-labels" id="barLabels">
            {cursos.map((c) => (
              <div
                className="bar-label"
                title={cursoLabel(c.nome)}
                key={c.nome}
              >
                {cursoLabel(c.nome)}
              </div>
      ...
      <section>
        ...
        <div className="donut-wrap">
          <div
            className="donut"
            style={grad ? { background: `conic-gradient(${grad})` } : undefined}
          >
          ...
          <ul className="legend">
            {pendencias.map((d) => (
              <li key={d.campo}>
                <span className="swatch" style={{ background: d.cor }} />
                <span className="l-label">{d.nome}</span>
                <span className="l-pct">
                  {pct(d.total, totalPend).toLocaleString("pt-BR", {
                    maximumFractionDigits: 1,
                  })}
                  %
                </span>
                <span className="l-value">{fmt(d.total)}</span>
              </li>
            ))}
          </ul>
        </div>
