...
function LayoutContent() {
  ...
  async function loadSheetsStatus() {
    ...
    try {
      ...
      if (!resposta.ok)
        throw new Error(dados.erro || "Falha ao verificar integração.");
  ...
  useEffect(() => {
    function outside(e: MouseEvent) {
      ...
      if (periodRef.current && !periodRef.current.contains(target))
        setPeriodOpen(false);
      if (integrationRef.current && !integrationRef.current.contains(target))
  ...
  if (erro || !periodoAtual) {
    return (
      <div style={{ padding: 32, fontFamily: "system-ui" }}>
        {erro || "Nenhum período cadastrado."}
      </div>
    );
  }
        ...
        <div className="main">
          <div className="topbar">
            ...
            >
              ...
              >
                Período ·{" "}
                <strong id="periodValue">{periodoAtual.codigo}</strong>
                    ...
                    return (
                      ...
                      >
                        ...
                        {selected ? (
                          ...
                          <span className="p-archived">arquivado</span>
                        ) : null}
                      </li>
                    );
                  })}
            ...
            <div
              className={`integration-picker${integrationOpen ? " open" : ""}`}
              id="integrationPicker"
              ref={integrationRef}
            >
              <div
                className="conn-status"
                id="integrationTrigger"
                onClick={() => setIntegrationOpen((v) => !v)}
              >
                <span
                  className="led"
                  style={
                    !connected && sheetsStatus
          ...
          {periodoAtual.status === "ARQUIVADO" && (
            ...
            >
              {modoApresentacao
                ? "Período arquivado em modo somente leitura."
                : "Você está visualizando um período arquivado."}
            </div>
          )}
